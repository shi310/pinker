...
class GameTertisController extends GetxController {
  final state = GameTertisState();
  Timer? downTimer;
  Timer? gameTimer;
  Timer? paushTimer;
  ...
  Future<void> init() async {
    ...
    await MyTimer.futureDelayed(300);
  }
  ...
  Future<void> remove(BoxData boxData) async {
    ...
    while (y >= boxData.origin.y) {
      if (isCanRemove(state.gameData.value.data[y])) {
        ...
        await MyTimer.futureDelayed(100, computation: () {
          state.gameData.update((val) {
            val!.data.removeAt(y);
  ...
  void addScore(int count) {
    ...
    if (state.score > state.scoreHistory) {
      state.scoreHistory = state.score;
      SharedService.to.setInt(sharedTetrisHistroyKey, state.scoreHistory);
    }
  ...
  @override
  void onReady() {
    debounce(state.isMoving, (callback) {
      if (callback) {
