  ...
  Widget build(BuildContext context) {
    ...
    final appBar = AppBar(
      ...
      leading: MyButton(
        ...
        child: MyIcons.back(context),
      ),
      title: Obx(
        () => Text(
          '正在播放: ${resourceData.value.name}',
        ),
      ),
    );

    /// 加载动画
    final lottie = MyIcons.lottieHolder();

    /// 视频区的遮罩，主要是遮住封面图
    final mark = Container(color: Colors.black.withOpacity(0.7));

    /// 视频加载动画只需要 0.5 的透明度，这里是处理透明度的
    final lottieOpcaty = Opacity(
      opacity: 0.5,
      child: MyIcons.lottieHolder(),
    );

    /// 加载中的组成部分
    final loadingContentChildren = <Widget>[
      MyIcons.loading(context),
    ...
    Widget buildVideoBox() {
      ...
      final loadingBox = Stack(children: [
        ...
        Positioned(top: 0, left: 0, bottom: 0, right: 0, child: mark),
        Positioned(top: 0, left: 0, bottom: 0, right: 0, child: lottieOpcaty),
        loadingContent,
      ]);
    ...
    /// 如果在加载中，那就是展示加载的样式
    /// 如果是在播放，那就展示播放的内容
    final videoBoxChild = Obx(buildVideoBox);

    /// 播放区域的样式
    const videoBoxChildDecortion =
        BoxDecoration(shape: BoxShape.rectangle, color: MyColors.black);
    ...
    Widget buildContentBox() {
      ...
      /// 收藏部分组件：喜欢构建
      Widget likeBuilder(bool _) => _ ? MyIcons.likePress() : MyIcons.like();
      ...
      Widget listViewBuild(int buttonIndex) {
        return Obx(() {
          ...
          void onTap() async {
            ...
            /// 然后传入新的播放地址，重新初始化播放器
            await controller.initPlayer(
              resourceData.value.playUrls[state.pageIndex].urls[buttonIndex],
            );

            // if (controller.VideoPlayViewerController.value.isInitialized) {
            //   /// 更换播放链接之前要把播放器控制器释放
            //   await controller.VideoPlayViewerController.dispose();
            ...
            //   /// 然后传入新的播放地址，重新初始化播放器
            //   await controller.VideoPlayView(
          ...
          /// 是否显示loading
          // const isShowLoading = state.isShowLoading;

          /// 是否在加载中
          // const isLoading = isChooise || isShowLoading;
